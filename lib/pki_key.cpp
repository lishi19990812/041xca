...
#include <openssl/core_names.h>  // 新增：OpenSSL 3.x Provider API 所需
...
{
#ifdef EVP_PKEY_SM2
	// 终极保底：如果是 SM2 类型，直接返回 NID_sm2，防止底层 API 失败
	if (getKeyType() == EVP_PKEY_SM2)
		return NID_sm2;
#endif

#if (OPENSSL_VERSION_NUMBER >= 0x30000000L)
	// OpenSSL 3.x 优先使用 Provider API
...
{
#if (OPENSSL_VERSION_NUMBER >= 0x30000000L)
	// OpenSSL 3.x 优先使用 Provider API 获取公钥点
...
#ifndef OPENSSL_NO_EC
	/* 绕过 getKeyType() 的坑，直接通过曲线 NID 判断 */
	int nid = ecParamNid();
	if (nid != NID_undef) {
		if (nid == NID_sm2) {
			nids << NID_sm3;
