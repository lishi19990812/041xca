...
int db_key::exportFlags(const QModelIndex &index) const
{
	...
	// 【删除这段代码，恢复 SM2 的传统 PEM 格式选项】
	// bool is_sm2 = false;
	// EVP_PKEY *pkey = key->getPubKey();
	// if (pkey) {
	// 	is_sm2 = EVP_PKEY_is_a(pkey, "SM2");
	// }
	// if (is_sm2)
	// 	disable_flags |= F_TRADITION;
			...
			XFile &file) const
{
	...
	if (xport->match_all(F_CRYPT)) {
		...
		// 【修改这里】：不管是不是 PKCS#8，只要是 SM2 就用 SM4，其他用 AES
		algo = is_sm2 ? EVP_sm4_cbc() : EVP_aes_256_cbc();
