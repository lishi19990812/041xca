...
#include <openssl/core_names.h>   // 新增这个头文件
#endif
...
void pki_evp::generate(const keyjob &task)
{
	...
	case EVP_PKEY_EC: 
	{
		EC_KEY *eckey = EC_KEY_new_by_curve_name(task.ec_nid);
		if (!eckey)
			break;
...
#ifdef EVP_PKEY_SM2
	case EVP_PKEY_SM2: {
		EC_KEY *eckey = EC_KEY_new_by_curve_name(task.ec_nid ? task.ec_nid : NID_sm2);
		if (!eckey)
			break;
		if (EC_KEY_generate_key(eckey)) {
			EVP_PKEY_assign_EC_KEY(key, eckey);
...
#if (OPENSSL_VERSION_NUMBER >= 0x30000000L)
	// OpenSSL 3.x: 直接向 Provider 询问私钥参数是否存在
	// 这是最准确的判断方法，不会误判公钥
	BIGNUM *priv_bn = NULL;
	if (EVP_PKEY_get_bn_param(key, OSSL_PKEY_PARAM_PRIV_KEY, &priv_bn)) {
		BN_free(priv_bn);
		return true;
	}
	// 对于 Ed25519 等没有 BIGNUM 形式私钥的密钥
	size_t len = 0;
...
void pki_evp::encryptKey(const char *password)
{
	...
	BioByteArray bba;
	i2d_PKCS8PrivateKey_bio(bba, key, ecParamNid() == NID_sm2 ? EVP_sm4_cbc() : EVP_aes_256_cbc(),
                                         ...
                                         pem_password_cb *cb, void *u)
{
	...
	pem = pem.trimmed(); // 清除可能存在的前后空格

	// 修复：如果是 SM2 或获取不到正确的名称，强制为 "EC PRIVATE KEY"
	if (pem.isEmpty() || pem == "PRIVATE KEY" || EVP_PKEY_id(x) == EVP_PKEY_SM2) {
		pem = "EC PRIVATE KEY";
	}
...
bool pki_evp::pem(BioByteArray &b, const pki_export *xport)
{
	...
	if (xport->match_all(F_PEM | F_PRIVATE)) {
		pkey = decryptKey();
		// 恢复 PEM 加密：如果选择了加密(F_CRYPT)，SM2使用SM4，其他使用AES-256
		const EVP_CIPHER *algo = xport->match_all(F_CRYPT) ?
			 (ecParamNid() == NID_sm2 ? EVP_sm4_cbc() : EVP_aes_256_cbc()) : NULL;
		PEM_write_bio_PrivateKey_traditional(b, pkey, algo,
...
void pki_evp::writeDefault(const QString &dirname) const
{
	...
	file.open_key();
    writeKey(file, pki_evp::passwd.isEmpty() ? NULL : 
             (ecParamNid() == NID_sm2 ? EVP_sm4_cbc() : EVP_aes_256_cbc()),
			mycb, true);
}
...
{
#ifndef OPENSSL_NO_EC
	// OpenSSL 3.x 对 SM2 密钥的内部验证函数支持不完善，直接跳过
