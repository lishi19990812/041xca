	...
	:QDialog(parent ? parent : mainwin)
{
	...
	keytype sm2_type;
	sm2_type.name = "SM2";
	sm2_type.type = EVP_PKEY_SM2;
	sm2_type.length = false;
	sm2_type.curve = true;
	sm2_type.mech = CKM_EC_KEY_PAIR_GEN;
	keytypes << keyListItem(sm2_type);
...
void NewKey::updateCurves(unsigned min, unsigned max, unsigned long ec_flags)
{
	...
	foreach(builtin_curve curve, builtinCurves) {
		...
		if (ec_flags && (curve.type & ec_flags) == 0)
			continue;
		if (curve.nid == NID_sm2)  // 新增：过滤掉 SM2，不显示在普通 EC 列表里
			continue;
		switch (curve.flags) {
...
void NewKey::on_keyType_currentIndexChanged(int idx)
{
	...
	} else if (ki.ktype.type == EVP_PKEY_EC) {
		// 新增：切回普通 EC 时，重新加载 EC 曲线列表
		updateCurves();
	} else if (ki.ktype.type == EVP_PKEY_SM2) {
		// 新增：选择 SM2 时，列表只显示 SM2
		curveBox->clear();
		const char *sn = OBJ_nid2sn(NID_sm2);
		curveBox->addItem(QString("%1: SM2 curve").arg(sn ? sn : "SM2"), NID_sm2);
	}
}
