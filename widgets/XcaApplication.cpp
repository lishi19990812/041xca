...
{
	QLocale lang = QLocale::system(); // 默认使用系统语言

	QFile file(defaultlang());
	if (file.open(QIODevice::ReadOnly)) {
		lang = QLocale(QString::fromUtf8(file.readAll()));
	}

	langAvail << QLocale::system();
	langAvail << QLocale("en");

	// 直接写死支持的语言列表，避免 QDir 扫描内存资源失败
	QStringList lang_codes = {
		"bg", "de", "es", "fa", "fr", "hr", "id", "it",
		"ja", "ko", "nl", "pl", "pt_BR", "ru", "sk", "tr", "zh_CN"
	};
	foreach(QString code, lang_codes) {
		langAvail << QLocale(code);
	}
