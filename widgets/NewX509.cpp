...
NewX509::NewX509(QWidget *w) : XcaDetail(w)
{
	...
	// --- 新增：动态添加自定义序列号 UI ---
	serialGroupBox = new QGroupBox(tr("序列号"), this);
	QHBoxLayout *serialLayout = new QHBoxLayout(serialGroupBox);

	customSerialCheckBox = new QCheckBox(tr("自定义序列号"), serialGroupBox);
	serialLineEdit = new QLineEdit(serialGroupBox);
	serialLineEdit->setPlaceholderText(tr("Copyright 041专属 © All Rights Reserved"));
	serialLineEdit->setEnabled(false); // 默认禁用，勾选后才启用

	// 设置验证器：只允许输入 0-9, A-F, a-f
	//QRegularExpression hexRegex("^[0-9A-Fa-f]*$");
	//QValidator *validator = new QRegularExpressionValidator(hexRegex, this);
	//serialLineEdit->setValidator(validator);

	serialLayout->addWidget(customSerialCheckBox);
	serialLayout->addWidget(serialLineEdit);

	// --- 强力插入逻辑：确保独占一整行 ---
	if (validityBox) {
		QWidget *parentWidget = validityBox->parentWidget();
		...
		if (gridLayout) {
			int index = gridLayout->indexOf(validityBox);
			...
			if (maxCol < 2) maxCol = 2; // 至少跨2列
			
			// 将 row 及其下方的所有控件向下移一行
			for (int i = gridLayout->count() - 1; i >= 0; --i) {
				...
				if (r >= row) {
					...
					gridLayout->addItem(item, r + 1, c, rs, cs);
				}
			}
			
			// 在腾出来的那一行，插入序列号组件并跨越所有列
			gridLayout->addWidget(serialGroupBox, row, 0, 1, maxCol);
		} 
		// 2. 如果不是网格布局，尝试向上寻找包含它们的垂直布局 (QVBoxLayout)
		else {
			QWidget *p = validityBox;
			QLayout *l = nullptr;
			while (p) {
				l = p->layout();
				if (l && qobject_cast<QVBoxLayout*>(l)) break;
				p = p->parentWidget();
			}
			
			if (l) {
				QVBoxLayout *vBoxLayout = qobject_cast<QVBoxLayout*>(l);
				// 找到 validityBox 或其直接父容器在这个垂直布局中的位置
				int idx = -1;
				for (int i = 0; i < vBoxLayout->count(); ++i) {
					QWidget *w = vBoxLayout->itemAt(i)->widget();
					if (w == parentWidget || w == validityBox) {
						idx = i;
						break;
					}
				}
				if (idx != -1) {
					// 插在 validityBox 所在层级的正前方
					vBoxLayout->insertWidget(idx, serialGroupBox);
				} else {
					// 保险起见的回退
					vBoxLayout->insertWidget(0, serialGroupBox);
				}
			} else {
				// 3. 实在找不到，追加在当前布局
				QBoxLayout *boxLayout = qobject_cast<QBoxLayout*>(parentLayout);
				if (boxLayout) {
					int idx = boxLayout->indexOf(validityBox);
					if (idx != -1)
						boxLayout->insertWidget(idx, serialGroupBox);
					else
						boxLayout->addWidget(serialGroupBox);
				}
			}
		}
	}
	...
	// 连接信号和槽
	connect(customSerialCheckBox, &QCheckBox::toggled, serialLineEdit, &QLineEdit::setEnabled);
	connect(serialLineEdit, &QLineEdit::textEdited, this, &NewX509::formatSerialInput);
...
// --- 新增：实现序列号输入格式化 ---
void NewX509::formatSerialInput()
{
    // 1. 获取文本并去掉所有冒号
    QString text = serialLineEdit->text();
    text.remove(':');
    
    // 2. 转换为大写
    text = text.toUpper();
    
    // 3. 使用正则表达式清除所有非 0-9, A-F 的字符
    text.replace(QRegularExpression("[^0-9A-F]"), "");
    ...
    // 5. 更新输入框内容并保持光标在最后
    serialLineEdit->blockSignals(true);
    serialLineEdit->setText(formatted);
    serialLineEdit->setCursorPosition(formatted.length());
    serialLineEdit->blockSignals(false);
}
...
bool NewX509::isCustomSerialEnabled() const
{
	return customSerialCheckBox && customSerialCheckBox->isChecked();
}

QString NewX509::getCustomSerialHex() const
{
	if (!isCustomSerialEnabled()) return QString();
	QString hex = serialLineEdit->text();
	hex.remove(':');
	if (hex.isEmpty()) return QString();
